...
import {
  ...
  createResourceType, updateResourceType, deleteResourceType,
} from '@/api/resourceTypes'
import type { ResourceTypeDto, CreateResourceTypePayload } from '@/api/resourceTypes'
...
export default function ResourceTypesPage() {
  ...
  const [sheetForm, setSheetForm] = useState<CreateResourceTypePayload>({ name: '', sortOrder: 0 })

  const [editingHeader, setEditingHeader] = useState(false)
  const [headerForm, setHeaderForm] = useState({ name: '', sortOrder: '' })

  // --- Queries ---
  const { data: types, isLoading: typesLoading } = useQuery<ResourceTypeDto[]>({
    queryKey: ['resource-types'],
    queryFn: getResourceTypes,
  })

  const { data: selected, isLoading: selectedLoading } = useQuery<ResourceTypeDto>({
    queryKey: ['resource-types', selectedId],
    queryFn: () => getResourceType(selectedId!),
  ...
  const createMutation = useMutation({
    ...
    onError: () => toast.error('Failed to create resource type'),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, ...payload }: { id: number } & Parameters<typeof updateResourceType>[1]) =>
      updateResourceType(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['resource-types'] })
      queryClient.invalidateQueries({ queryKey: ['resource-types', selectedId] })
  ...
  return (
    <div className="space-y-4 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage the list of resource types available for selection in journeys and catalog entries</p>
      </div>
        ...
        <div className="w-72 shrink-0 flex flex-col gap-3">
          ...
          <div className="rounded-md border overflow-hidden flex-1 overflow-y-auto">
            {typesLoading ? (
              ...
              <div className="p-4 text-sm text-muted-foreground text-center">No resource types yet.</div>
            ) : (
              types.map((t) => (
                <div
                  key={t.id}
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          {selectedId === null ? (
            <div className="flex-1 rounded-md border flex items-center justify-center">
              <p className="text-sm text-muted-foreground">Select a resource type to view details</p>
            </div>
          ) : (
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent>
          <SheetHeader>
            <SheetTitle>New Resource Type</SheetTitle>
          </SheetHeader>
          <form onSubmit={handleSheetSubmit} className="space-y-4 mt-4">
            <div className="space-y-1.5">
              <Label htmlFor="rt-name">Name <span className="text-destructive">*</span></Label>
              <Input
                id="rt-name"
                value={sheetForm.name}
                onChange={e => setSheetForm(p => ({ ...p, name: e.target.value }))}
                placeholder="e.g. GoogleSlides"
                autoFocus
              />
            </div>
            <div className="space-y-1.5">
